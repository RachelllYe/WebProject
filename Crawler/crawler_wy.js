...
// var date_format = "$('.pub_time').text()";
var date_format =" $('meta[property=\"article:published_time\"]').eq(0).attr(\"content\")";

var content_format = " $('meta[name=\"description\"]').eq(0).attr(\"content\")";
...
function seedget() {
    request(seedURL, function(err, res, body) { //读取种子页面
        ...
        if (seedurl_news == undefined) console.log('yes is null');
...
function newsGet(myURL) { //读取新闻页面
    request(myURL, function(err, res, body) { //读取新闻页面
        ...
        if (date_format != "") fetch.publish_date = fetch.publish_date = new Date("2022-07-21").toFormat("YYYY-MM-DD");//刊登日期   
